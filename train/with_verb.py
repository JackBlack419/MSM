...
from model.with_verb_block import With_verb_block # 导入With_verb_block模型
from utils_eval.utils import verb_dataloader, LabelSmoothing
...
log_dir = os.path.join(opt.log_dir,'verb_log',backbone) # 设置保存log的路径
os.makedirs(log_dir,exist_ok=True) # 创建该路径
verb_loss_writer = SummaryWriter(log_dir=log_dir) # 设置一个SummaryWriter便于在tensorboard上查看
...
MSRVTT_model_weights_path = os.path.join(opt.model_weights_path,'verb',backbone,'MSRVTT') # 设置在MSRVTT训练后的权重保存路径
MSVD_model_weights_path = os.path.join(opt.model_weights_path,'verb',backbone,'MSVD') # 设置在MSVD训练后的权重保存路径
...
model = With_verb_block(MSRVTT_output_dim,d_model,d_ff,n_layers,heads,PAD_ID,dropout_rate,device)
...
def loss_func(base_pred, verb_pred, tgt, verbs_tgt):
    # 将预测值展平为二维张量
    ...
    verb_pred_flat = verb_pred.contiguous().view(-1, verb_pred.size(-1))
    verb_tgt_flat = verbs_tgt.contiguous().view(-1)
    verb_loss = criterion_MSRVTT(verb_pred_flat, verb_tgt_flat)  / verbs_tgt.size(0)
    # print('===================================')
    # print(base_loss) # tensor(93.8767, device='cuda:0', grad_fn=<DivBackward0>)
    # print('===================================')
    # print(verb_loss) # tensor(38.0608, device='cuda:0', grad_fn=<DivBackward0>)
    # ss = input("press enter to continue")

    loss =  base_loss + verb_loss
    return loss
...
for epoch in tqdm(range(MSRVTT_num_epochs)):
    ...
    count = 0
    train_data_loss = 0
    for src, tgt, verb_q, verb_tgt in verb_dataloader(MSRVTT_feature_path, os.path.join(opt.captions_path,'MSRVTT.h5'),
                                                            backbone, 'train', batch_size):
        src, tgt, verb_q, verb_tgt = src.to(device), tgt.to(device), verb_q.to(device), verb_tgt.to(device) # 将数据移到设备上
        base_input = tgt[:, :-1]
        verb_input = verb_q[:, :-1]
        
        optimizer.zero_grad()  # 清零梯度
        with autocast(device_type='cuda', dtype=torch.float16):
            outputs, verb_outs = model(src, base_input, verb_input)  # 前向传播
            loss = loss_func(model.generator(outputs), model.generator(verb_outs), tgt[:, 1:], verb_tgt[:, 1:])  # 计算损失
    ...
    verb_loss_writer.add_scalar("MSRVTT_train_loss", train_data_loss, epoch)
    ...
    with torch.no_grad():
        ...
        for src, tgt, verb_q, verb_tgt in verb_dataloader(MSRVTT_feature_path, os.path.join(opt.captions_path,'MSRVTT.h5'),
            ...
            with autocast(device_type='cuda', dtype=torch.float16):
                outputs,verb_outs = model(src, base_input, verb_input)
                loss = loss_func(model.generator(outputs), model.generator(verb_outs), tgt[:, 1:], verb_tgt[:, 1:])  # 计算损失
            valid_data_loss_ += loss
            n_count += 1

        valid_data_loss_ = valid_data_loss_ / n_count
    verb_loss_writer.add_scalar("MSRVTT_valid_loss", valid_data_loss_, epoch)


# 在MSVD数据集上的训练
MSVD_output_dim = vocab_length
model = With_verb_block(MSVD_output_dim,d_model,d_ff,n_layers,heads,PAD_ID,dropout_rate,device)
...
def loss_func(base_pred, verb_pred, tgt, verbs_tgt):
    ...
    verb_pred_flat = verb_pred.contiguous().view(-1, verb_pred.size(-1))
    verb_tgt_flat = verbs_tgt.contiguous().view(-1)
    verb_loss = criterion_MSVD(verb_pred_flat, verb_tgt_flat)  / verbs_tgt.size(0)
    # print('===================================')
    # print(base_loss) # tensor(93.8767, device='cuda:0', grad_fn=<DivBackward0>)
    # print('===================================')
    # print(verb_loss) # tensor(38.0608, device='cuda:0', grad_fn=<DivBackward0>)
...
for epoch in tqdm(range(MSVD_num_epochs)):
    ...
    count = 0
    train_data_loss = 0
    for src, tgt, verb_q, verb_tgt in verb_dataloader(MSVD_feature_path, os.path.join(opt.captions_path,'MSVD.h5'),
                                                            backbone, 'train', batch_size):
        src, tgt, verb_q, verb_tgt = src.to(device), tgt.to(device), verb_q.to(device), verb_tgt.to(device) # 将数据移到设备上
        base_input = tgt[:, :-1]
        verb_input = verb_q[:, :-1]
    ...
    train_data_loss = train_data_loss / count
    verb_loss_writer.add_scalar("MSVD_train_loss", train_data_loss, epoch)
    print(f"第{epoch + 1}轮的loss: {loss}")
    ...
    with torch.no_grad():
        ...
        valid_data_loss_ = valid_data_loss_ / n_count
    verb_loss_writer.add_scalar("MSVD_valid_loss", valid_data_loss_, epoch)