...
from utils_eval.utils import load_src_tgt, LabelSmoothing
...
log_dir = os.path.join(opt.log_dir,'baseline_log',backbone) # 设置保存log的路径
os.makedirs(log_dir,exist_ok=True) # 创建该路径
baseline_loss_writer = SummaryWriter(log_dir=log_dir) # 设置一个SummaryWriter便于在tensorboard上查看
...
def loss_func(pred, tgt):
    ...
    # 计算损失
    loss = criterion_MSRVTT(pred_flat, tgt_flat) / tgt.size(0)
    return loss
...
for epoch in tqdm(range(MSRVTT_num_epochs)):
    ...
    count = 0
    train_data_loss = 0
    for src, tgt in load_src_tgt(MSRVTT_feature_path, os.path.join(opt.captions_path,'MSRVTT.h5'), backbone, 'train', batch_size):
        src, tgt = src.to(device), tgt.to(device)  # 将数据移到设备上
        y_input = tgt[:, :-1]
        
        optimizer.zero_grad()  # 清零梯度
        with autocast(device_type='cuda', dtype=torch.float16):
            outputs = model(src, y_input)  # 前向传播
            loss = loss_func(model.generator(outputs), tgt[:, 1:])  # 计算损失
    ...
    baseline_loss_writer.add_scalar("MSRVTT_train_loss", train_data_loss, epoch)
    ...
    with torch.no_grad():
        valid_data_loss_ = 0
        n_count = 0
        for src, tgt in load_src_tgt(MSRVTT_feature_path, os.path.join(opt.captions_path,'MSRVTT.h5'), backbone, 'valid', batch_size):
            src, tgt = src.to(device), tgt.to(device)  # 将数据移到设备上
            y_input = tgt[:, :-1]

            outputs = model(src, y_input)
            loss = loss_func(model.generator(outputs), tgt[:, 1:])  # 计算损失
            valid_data_loss_ += loss
            n_count += 1

        valid_data_loss_ = valid_data_loss_ / n_count
    baseline_loss_writer.add_scalar("MSRVTT_valid_loss", valid_data_loss_, epoch)
...
def loss_func(pred, tgt):
    ...
    tgt_flat = tgt.contiguous().view(-1)
    # 计算损失
    loss = criterion_MSVD(pred_flat, tgt_flat) / tgt.size(0)
    return loss
...
for epoch in tqdm(range(MSVD_num_epochs)):
    ...
    count = 0
    train_data_loss = 0
    for src, tgt in load_src_tgt(MSVD_feature_path, os.path.join(opt.captions_path,'MSVD.h5'), backbone, 'train', batch_size):
        src, tgt = src.to(device), tgt.to(device)  # 将数据移到设备上
        y_input = tgt[:, :-1]
    ...
    train_data_loss = train_data_loss / count
    baseline_loss_writer.add_scalar("MSVD_train_loss", train_data_loss, epoch)
    print(f"第{epoch + 1}轮的loss: {loss}")
    ...
    with torch.no_grad():
        valid_data_loss_ = 0
        n_count = 0
        for src, tgt in load_src_tgt(MSVD_feature_path, os.path.join(opt.captions_path,'MSVD.h5'), backbone, 'valid', batch_size):
            src, tgt = src.to(device), tgt.to(device)  # 将数据移到设备上
            y_input = tgt[:, :-1]
        ...
        valid_data_loss_ = valid_data_loss_ / n_count
    baseline_loss_writer.add_scalar("MSVD_valid_loss", valid_data_loss_, epoch)