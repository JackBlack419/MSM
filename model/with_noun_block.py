...
class With_noun_block(nn.Module):
    ...
    def encode(self,src):
        src = self.base_encode(src) + self.noun_encode(src)
        return src
    ...
    def noun_decode(self,noun_src,noun_tgt):

        # 生成一个遮蔽mask，用于屏蔽padding位置的信息
        noun_tgt_pad_mask = noun_tgt == self.pad_idx
        noun_tgt_pad_mask = noun_tgt_pad_mask.to(noun_tgt.device)
        # 生成一个遮蔽mask，用于屏蔽未来位置的信息
        noun_tgt_mask = self.generate_square_subsequent_mask(noun_tgt.shape[1], device=noun_tgt.device)
        
        # 将token序列转换为embedding表示，并加上位置编码
        noun_tgt_embeded = self.position_embedding(self.caption_embedding(noun_tgt))
        noun_out = self.noun_decoder(noun_tgt_embeded, noun_src, noun_tgt_mask, None, noun_tgt_pad_mask, None)

        return noun_out
    # 整体前向传播流程
    def forward(self, src, tgt, noun_tgt):
        finall_src = self.encode(src)
        noun_src = self.noun_encode(src)
        out = self.decode(finall_src,tgt)
        noun_out = self.noun_decode(noun_src,noun_tgt)
        return out, noun_out
