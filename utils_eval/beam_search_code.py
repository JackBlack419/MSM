...
def beam_search(model, src, max_len, pad, bos, eos, beam_size, device):
    ...
    def beam_decode_step(
        ...
        def predict_word(dec_seq, enc_output, n_active_inst, n_bm):
            assert enc_output.shape[0] == dec_seq.shape[0]
            tgt_mask = model.generate_square_subsequent_mask(dec_seq.size(1), device=device)
            tgt_embeded = model.position_embedding(model.caption_embedding(dec_seq))
            with autocast(device_type='cuda', dtype=torch.float16):
                out = model.decoder(tgt_embeded, enc_output, tgt_mask=tgt_mask)
